...
int[] num_circles = {5,5,5,5,5};
float[] arc_size = { HALF_PI, HALF_PI,HALF_PI,HALF_PI,HALF_PI };
...
public void draw() {
  ...
  for( int i = 0; i < rotation_offset.length; ++i ) {
    drawCircles( SIZE - RADIAL_SPACING  * i, rotation_offset[i], num_circles[i], arc_size[i]);
  }
 ...
 */
public void drawCircles( float radius, float rotation_offset, int number_circles, float arc_size) {
  ...
  // rounding
  float radians_between_circles = arc_size / (float) segments_between_circles - 0.1;
  float arc_length = arc_size * radius;
  float circle_radius = (arc_length - segments_between_circles * SPACING * radius) / (number_circles * 2.0f);

  for( float degree = rotation_offset; degree <= arc_size + rotation_offset; degree += radians_between_circles ) {
